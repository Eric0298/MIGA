...
public sealed class DemoCookieEvents : CookieAuthenticationEvents
{
    ...
    public override async Task ValidatePrincipal(CookieValidatePrincipalContext context)
    {
        ...
        if (now - session.LastSeenAtUtc >= LastSeenWriteInterval)
        {
            ...
            session.IdleExpiresAtUtc = Min(now + _options.DemoIdleTimeout, session.AbsoluteExpiresAtUtc);
    ...
    private static async Task RejectAsync(CookieValidatePrincipalContext context)
    {
        ...
        await context.HttpContext.SignOutAsync(MigaAuthenticationConstants.DemoScheme);
    }
}

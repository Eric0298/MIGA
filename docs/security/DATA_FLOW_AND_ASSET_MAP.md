...
| Usuario registrado          | ASP.NET Core Identity y sesión servidor | snapshot propio, YouTube, cambio de contraseña, exportación y eliminación                           |
...
| IndexedDB `miga-scoped-<hash>`       | navegador                       | personal                          | identidad activa                                         | se purga al cambiar/cerrar/eliminar sesión según flujo cliente                |
...
- elimina bases scoped válidas al cerrar/eliminar sesión y al cambiar de identidad;
...
`Persistence/Migrations/20260723155637_InitialSecurity.cs` y su model snapshot. La migración no
contiene seed de datos ni credenciales.
...
| ------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| PostgreSQL                | identidad, sesiones, snapshots, auditoría, keyring                                | EF parametrizado, cuenta de aplicación                                        | TLS/cifrado/backup del proveedor                                  |
| SMTP                      | dirección y enlace de confirmación/restablecimiento                               | abstracción, activación explícita                                             | proveedor, DPA, credenciales, SPF/DKIM/DMARC                      |
...
| GitHub Actions            | código y metadatos de build                                                       | permisos mínimos, Actions por SHA                                             | ejecutar y proteger checks                                        |
...
| Entorno                  | Ejecución/persistencia                                                      | Diferencias y límites                                                                                              |
| ------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
...
| Test unitario/frontend   | Vitest, jsdom y `fake-indexeddb`; datos ficticios                           | no valida TLS, proxy, PostgreSQL ni headers del hosting                                                            |
| Test integración backend | `WebApplicationFactory`, SQLite en memoria y remitente de correo falso      | prueba lógica HTTP/Identity, pero no reproduce todas las diferencias Npgsql/PostgreSQL                             |
| CI                       | runners GitHub efímeros y, cuando corresponda, PostgreSQL fijado por digest | configuración versionada; no equivale a producción ni debe recibir secretos de producción                          |
| Producción               | infraestructura todavía no definida                                         | requiere HTTPS, gestor de secretos, PostgreSQL/backup, certificado, SMTP, observabilidad y validación de checklist |
...
3. El servidor normaliza el email, aplica política de contraseña y crea Identity/workspace/sesión en
   una transacción.
4. Si se importa demo, valida que la cookie corresponde a ese workspace, copia el snapshot y elimina
   el workspace demo.
5. Si se exige confirmación, SMTP envía un enlace construido desde `PublicBaseUrl`.
...
4. `PUT` envía revisión esperada y JSON; el servidor revalida límites, claves, UUID y relaciones.
...
| `POST /api/auth/register`             | público                                          | email, contraseña, versión privacidad, `importDemoData` | `204` o `202`                                         | 5/h/IP                | email único normalizado; rol no aceptado                                                 |
...
| `POST /api/auth/confirm-email`        | público                                          | `userId`, token                                         | `204`                                                 | 5/h/IP                | token limitado                                                                           |
...
| `PUT /api/data/snapshot`              | demo o registrado                                | revisión y JSON                                         | snapshot actualizado                                  | 30/min/IP             | 256 KiB demo, 5 MiB registrado; `409` optimista                                          |
...
“Añadida” significa creada durante esta implementación; no implica que el workflow remoto haya
terminado correctamente. El resultado de ejecución se registra en la auditoría.
...
| `POST /api/auth/confirm-email`        | añadida: éxito, expiración y un solo uso                                                                               |
...
| `PUT /api/data/snapshot`              | añadida: aislamiento, `409`, campos desconocidos, tamaño y validación estructural unitaria                             |
...
| Cuenta                    | hasta eliminación por el usuario                                                                    |
...
| Blobs locales             | hasta borrado del scope/navegador; sin copia servidor                                               |
